...
def run_ridgereg_conformal(dataset,misspec = False):
    #Compute intervals
    #Initialize
    train_frac = 0.7
    ...
    beta_post = jnp.load("samples/beta_post_ridgereg_{}.npy".format(suffix))
    intercept_post = jnp.load("samples/intercept_post_ridgereg_{}.npy".format(suffix))
    sigma_post = jnp.load("samples/sigma_post_ridgereg_{}.npy".format(suffix))
    ...
    np.save("results/region_cb_ridgereg_{}".format(suffix),region_cb)
    np.save("results/band_bayes_ridgereg_{}".format(suffix),band_bayes)
    np.save("results/band_split_ridgereg_{}".format(suffix),band_split)
    np.save("results/region_full_ridgereg_{}".format(suffix),band_split)

    np.save("results/coverage_cb_ridgereg_{}".format(suffix),coverage_cb)
    np.save("results/coverage_cb_exact_ridgereg_{}".format(suffix),coverage_cb_exact)
    np.save("results/coverage_bayes_ridgereg_{}".format(suffix),coverage_bayes)
    np.save("results/coverage_split_ridgereg_{}".format(suffix),coverage_split)
    np.save("results/coverage_full_ridgereg_{}".format(suffix),coverage_full)

    np.save("results/length_cb_ridgereg_{}".format(suffix),length_cb)
    np.save("results/length_bayes_ridgereg_{}".format(suffix),length_bayes)
    np.save("results/length_split_ridgereg_{}".format(suffix),length_split)
    np.save("results/length_full_ridgereg_{}".format(suffix),length_full)

    np.save("results/times_cb_ridgereg_{}".format(suffix),times_cb)
    np.save("results/times_bayes_ridgereg_{}".format(suffix),times_bayes)
    np.save("results/times_split_ridgereg_{}".format(suffix),times_split)
    np.save("results/times_full_ridgereg_{}".format(suffix),times_full)
